...
def inference_ddpm(
    ...
    num_train_steps = model.module.noise_scheduler.max_timesteps
    ...
    if visualize:
        ...
        all_pred = []
        noise_samples = []
    image_features = model.module.feature_extractor(images, autoresize=True)
    if model.module.append_ndc:
        # (B, N, H, W, 3)
        ...
        ndc_coordinates = ndc_coordinates.squeeze(0)
        ndc_coordinates = ndc_coordinates.to(device, non_blocking=True)[..., :2]
        # (B, N, 2, H, W)
    ...
    for t in loop:
        ...
        if pred_x0:
            ...
            if normalize_moments:
                dot_product = (d * m).sum(dim=2, keepdim=True)
                normalized_m = m - dot_product * d / c
            normalized_eps_pred = torch.cat((normalized_d, normalized_m), dim=2)
            eps_pred = normalized_eps_pred

        if visualize:
            all_pred.append(eps_pred.clone())
            noise_samples.append(noise_sample)

        alpha_t = model.module.noise_scheduler.alphas[t]
        alpha_bar_t = model.module.noise_scheduler.alphas_cumprod[t]
        if t == 0:
            alpha_bar_tm1 = torch.ones_like(alpha_bar_t)
        else:
            alpha_bar_tm1 = model.module.noise_scheduler.alphas_cumprod[t - 1]
        beta_t = model.module.noise_scheduler.betas[t]
        if beta_tilde:
