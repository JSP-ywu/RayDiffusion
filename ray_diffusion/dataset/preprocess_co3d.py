"""
Script to pre-process camera poses and bounding boxes for CO3Dv1 dataset. This is
...
def get_parser():
    ...
    parser.add_argument("--category", type=str, default="tv")
    parser.add_argument("--output_dir", type=str, default="dataset/co3dv2_annotations")
    parser.add_argument("--co3d_dir", type=str, default="/home/vision/pjs/co3d_v2")
    parser.add_argument(
        "--min_quality",
        type=float,
        default=0.5,
...
def process_poses(co3d_dir, category, output_dir, min_quality):
    ...
    #subset_lists = ["train", "test"]
    subset_lists_train_file = osp.join(category_dir, "set_lists", "set_lists_fewview_train.json")
    subset_lists_test_file = osp.join(category_dir, "set_lists", "set_lists_fewview_test.json")

    bbox_file = osp.join(args.output_dir, f"{category}_bbox.jgz")
    subset_lists_data = {}
    with open(subset_lists_train_file) as f:
        subset_lists_data['train'] = json.load(f)['train']

    with open(subset_lists_test_file) as f:
        subset_lists_data['test'] = json.load(f)['test']
    ...
    for subset in subset_lists_data.keys():
        category_data = {}  # {sequence_name: [{filepath, R, T}]}
        #print(type(subset_lists_data[subset][subset]))
